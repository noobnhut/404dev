<template>
    ...
    >
      <div class="flex flex-wrap -mx-3">
        ...
        <div class="flex-none w-auto max-w-full px-3 my-auto">
          <div class="h-full">
            <h5 class="mb-1">Quản lý đường dẫn</h5>
          </div>
        </div>
        <div
  ...
  <div class="flex items-center justify-between mb-4">
    <h2 class="text-xl font-bold text-gray-800">Quản lý đường dẫn</h2>
  </div>
    ...
    >
      ...
      <div class="px-4">
        ...
        <div>
          ...
          <p class="text-red-500 text-sm ml-1" v-if="!status && statusFocused">
            Không được để trống.
          </p>
        </div>
  ...
  >
    ...
    >
      <div class="flex flex-row py-3 px-4">
        <h5 class="text-lg font-semibold flex-grow">Cập nhập đường dẫn</h5>
        <i
          class="uil-multiply flex-none cursor-pointer bg-gray-400 rounded-xl"
          @click="openShowUpdate"
        ></i>
  ...
  >
    ...
    >
      <div class="flex flex-row py-3 px-4">
        <h5 class="text-lg font-semibold flex-grow">Xóa đường dẫn</h5>
        <i
          class="uil-multiply flex-none cursor-pointer bg-gray-400 rounded-xl"
          @click="openShowDelete"
        ></i>
...
export default {
  data() {
    return {
      ...
      links: [],
      link: "",
      titleFocused: false, contentFocused: false, statusFocused: false
    };
  },
  mounted() {
    this.getLink();
  ...
  methods: {
    async getLink() {
      try {
        const result = await this.$axios.get("about_link/get");
        if (result.data.success == true) {
          this.links = result.data.result;
        }
    ...
    async addLink() {
      try {
        ...
        if (this.title && this.content && this.status) {
          const result = await this.$axios.post("about_link/add", {
            title: this.title,
    ...
    async updateLink() {
      try {
        ...
        this.statusFocused = true;
        if (this.title && this.content && this.status) {
          const result = await this.$axios.put(`about_link/update/${this.link.id}`, {
            title: this.title,
            content: this.content,
            status: this.status
          });
    ...
    async deleteLink() {
      const result = await this.$axios.delete(`about_link/delete/${this.link.id}`);
      if (result.data.success == true) {
        this.$refs.toast.showToast(result.data.message);
        setTimeout(() => {
          this.getLink();
